...
import { journalLoanReceived, journalLoanInterest } from "@/lib/autoJournal";
...
const LoanBorrowedPage = () => {
  ...
  const saveLoan = useMutation({
    mutationFn: async () => {
      ...
      return loan as { id: string; start_date: string; opening_balance: number; loan_type: string };
    },
    onSuccess: async (data) => {
  if (data) {
    await journalLoanReceived(data.id, data.start_date, data.opening_balance, data.loan_type);
  }
  ...
  // ✅ saveTx — interest journal সহ
  const saveTx = useMutation({
    mutationFn: async () => {
      const { data: tx, error } = await supabase.from("loan_transactions").insert({
   ...
   onSuccess: async (data) => {
  if (data && txType === "interest_paid") {
    await journalLoanInterest(data.id, txDate, txAmount, activeYear?.id);
  }
      toast({ title: "লেনদেন সংরক্ষিত ✅" });
      qc.invalidateQueries({ queryKey: ["loan-borrowed-all-tx"] });
      setTxOpen(false); setTxAmount(0); setTxNote("");
    },
    onError: (e: any) => toast({ title: "ত্রুটি", description: e.message, variant: "destructive" }),
  });
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <div className="bg-card border rounded-xl px-4 py-3">
          <p className="text-xs font-bengali text-muted-foreground mb-1">লোনের ফাইল সংযুক্তি</p>
          <EntryAttachment module="loan" entryId={selectedLoan} entryLabel={selectedLoanData.party_name} />
        </div>
        ...
        <Card>
          <CardContent className="pt-4">
            ...
            <Table>
              ...
              <TableBody>
                {selectedTx.length === 0 && (
                  <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground">কোনো লেনদেন নেই</TableCell></TableRow>
                )}
                {selectedTx.map((tx) => (
                  <>
                    <TableRow key={tx.id}>
                      ...
                      <TableCell>
                        {tx.type === "repay_principal"
                          ? <Badge variant="outline" className="text-green-700">কিস্তি</Badge>
                          : <Badge variant="outline" className="text-amber-700">সুদ</Badge>}
                      </TableCell>
                    ...
                    <TableRow key={`${tx.id}-attach`}>
                      <TableCell colSpan={4} className="px-3 pb-2 pt-0">
                        <EntryAttachment module="loan" entryId={tx.id} />
                      </TableCell>
                    </TableRow>
                  </>
                ))}
            ...
            {loans.filter((l) => l.loan_type === t.key).map((loan) => {
              ...
              return (
                <Card key={loan.id} className="hover:shadow-md transition-all">
                  <CardContent className="pt-4">
                    <div className="flex justify-between items-start cursor-pointer" onClick={() => setSelectedLoan(loan.id)}>
                      <div>
                    ...
                    <div className="mt-2" onClick={(e) => e.stopPropagation()}>
                      <EntryAttachment module="loan" entryId={loan.id} entryLabel={loan.party_name} />
                    </div>
                  </CardContent>
                </Card>
              );
