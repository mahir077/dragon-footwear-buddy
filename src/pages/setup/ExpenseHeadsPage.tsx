...
const ExpenseHeadsPage = () => {
  ...
  const { data: heads = [] } = useQuery({
    queryKey: ["expense_heads_all"],
    queryFn: async () => {
      const { data } = await supabase.from("expense_heads").select("*").order("created_at");
      return data || [];
    },
  });
...
const { error } = await supabase.from("expense_heads").insert({ name_bn: nameBn });
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["expense_heads_all"] });
      qc.invalidateQueries({ queryKey: ["expense_heads"] });
      setNameBn(""); setNameEn("");
      toast({ title: "ব্যয়ের খাত যোগ হয়েছে ✅" });
    },
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-bengali">ব্যয়ের খাত</h1>
        <p className="text-sm text-muted-foreground">Expense Heads</p>
      </div>
      ...
      <div className="bg-card border rounded-xl p-5 space-y-3">
        ...
        <button onClick={() => addMutation.mutate()}
          ...
          className="w-full bg-[hsl(var(--stat-red))] text-white px-6 py-3 rounded-xl text-base font-bengali font-bold hover:opacity-90 disabled:opacity-50">
